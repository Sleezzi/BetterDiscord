...
const showChangeLog = async () => {
    if (!window.sleezzi || window.sleezzi.state !== "active")
        return;
    if (await Data.load("version") === config.version)
        return;
    ...
    window.sleezzi.alert({
        ...
        subtitle: `${formatter.format(config.changelogDate)} - v${config.version}`,
        sections: config.changelog.map((section) => {
...
const installPlugin = async (name, url) => {
...
const plugin = () => ({
    start: async () => {
        ...
        if (window.sleezzi.state === "installing")
            return;
        await showChangeLog();
